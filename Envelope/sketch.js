...
function setup() {
    ...
    env = new Envelope(windowWidth / 2, windowHeight / 2);
...
class Envelope{
    ...
    display(nLines){
        ...
        line(this.x, this.y, this.x, this.y - this.axisLength);
        line(this.x, this.y, this.x + this.axisLength, this.y);

        let spacing = this.axisLength / (nLines + 1)

        for(let i = 0; i <= nLines; i++)
            line(this.x + this.axisLength - spacing * i, this.y, this.x, this.y - spacing * i);
...
function createUI(){
    ...
    sliderNumberLines.position(15, windowHeight - 35);
